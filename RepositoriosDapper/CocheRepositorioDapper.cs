    ...
    public class CocheRepositorioDapper : ICocheRepositorio
{
    ...
    public CocheRepositorioDapper(IConfiguration configuration, IMapper mapper, BBDDContext contexto) // constructor
    {
    ...
    public async Task<IEnumerable<CocheDTO>> ObtenerTodosCoches()
    {
        ...
        var coches = await connection.QueryAsync<Coche>(sql);
    ...
    public async Task<Coche> ObtenerCochePorId(int id) // no hago con automapper porque el DTO no tiene las propiedades de navegación.
    {
    ...
    public async Task AgregarCoche(CocheDTO cocheDTO)     // DTO -> Entidad (para guardar) 
    {
        var cocheMapeado = _mapper.Map<Coche>(cocheDTO);  //  CocheDTO -> Coche

        using var connection = new SQLiteConnection(_connectionString);

        var sql = @"
        INSERT INTO Coches (Marca, Modelo, Descripcion, Patente, Tipo_coche, Tipo_cambio,
                            Num_plazas, Num_maletas, Num_puertas, Posee_aire_acondicionado,
                            Imagen, GrupoId, SucursalId)
                    VALUES 
                            (@Marca, @Modelo, @Descripcion, @Patente, @Tipo_coche, @Tipo_cambio,
                            @Num_plazas, @Num_maletas, @Num_puertas, @Posee_aire_acondicionado,
                            @Imagen, @GrupoId, @SucursalId);
                ";

        await connection.ExecuteAsync(sql, cocheMapeado);
    }
